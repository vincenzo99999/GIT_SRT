...
void *activate_backup(void * par) {
    ...
    mqd_t backup_qd;
    if ((backup_qd = mq_open(BACKUP_QUEUE_NAME, O_RDONLY | O_CREAT | O_NONBLOCK, QUEUE_PERMISSIONS, &attr)) == -1) {
        perror("activate_backup: mq_open (backup)");
        exit(1);
    }
    ...
    while (keep_on_running) {
        ...
        // Prova a ricevere il messaggio
        if (mq_receive(backup_qd, message, MAX_MSG_SIZE, NULL) != -1) {
            // Se arriva qualcosa, attiva il controller di backup
            active = 1;
            printf("[BACKUP] Activation signal received, active = 1\n"); // DEBUG
        }
    }

    if (mq_close(backup_qd) == -1) {
        perror("activate_backup: mq_close backup_qd");
        exit(1);
    }
...
int main(void)
{
    ...
    pthread_t actuator_thread;
	    // ACTIVATE BACKUP THREAD
	periodic_thread activation_th;
	pthread_attr_t myattrA;
	struct sched_param myparamA;
	activation_th.period = TICK_TIME;  // Frequenza di polling della coda di backup
	activation_th.priority = 40;
	myparamA.sched_priority = activation_th.priority;

	pthread_attr_init(&myattrA)
	pthread_attr_setschedparam(&myattrA, &myparamA);
	pthread_attr_setschedpolicy(&myattrA,SCHED_FIFO);
	pthread_attr_setinheritsched(&myattrA,PTHREAD_EXPLICIT_SCHED) 
	pthread_create(&activation_thread, &myattrA, activate_backup, (void*)&activation_th);
